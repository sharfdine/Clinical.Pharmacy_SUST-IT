...
with st.expander("🔧 Sheet & Column Mapping", expanded=False):
    ...
    for stype, sname in final_mapping.items():
        # A summary-style ADR sheet (a few label/value pairs like "Total
        # ADRs Reported: 12") isn't a row-per-incident table, so the
        # per-column dropdowns below don't apply to it and would just be
        # confusing — show the three plain figures instead.
        if stype == "adr":
            adr_summary_preview = try_read_adr_summary(BytesIO(file_bytes), sname)
            if adr_summary_preview:
                with st.expander(f"Columns for: {sname} ({stype})"):
                    st.info(
                        "This ADR sheet was read as a summary block (not a "
                        "row-per-incident table), so there are no columns to map. "
                        "The figures found on the sheet are:"
                    )
                    st.markdown(
                        f"- **Total ADRs:** {adr_summary_preview['total_adrs']}\n"
                        f"- **HAM/LASA-related ADRs:** {adr_summary_preview['adrs_from_ham_lasa']}\n"
                        f"- **Other ADRs:** {adr_summary_preview['adrs_other']}"
                    )
                col_mappings[stype] = {}
                continue
...
if "ham_lasa" in final_mapping:
    ...
    ham_patient_df = ham_lasa_by_patient(df_hl, cmap_hl)

# ── ADR Analysis ─────────────────────────────────────────────────────
if "adr" in final_mapping:
    adr_summary = try_read_adr_summary(BytesIO(file_bytes), final_mapping["adr"])
    if adr_summary:
        adr_results = {
            "total_adrs": adr_summary["total_adrs"],
            "adrs_from_ham_lasa": adr_summary["adrs_from_ham_lasa"],
            "adrs_other": adr_summary["total_adrs"] - adr_summary["adrs_from_ham_lasa"],
...
with tab2:
    ...
    if error_results and error_results["total_errors"] > 0:
        ...
        with k2:
            render_kpi("Prescription", error_results["prescription"], "#4F8FEA")
        with k3:
            render_kpi("Administration", error_results["administration"], "#F39C12")
        with k4:
        ...
        with c1:
            ...
            render_description(
                ...
                "Prescription errors typically include dose, frequency, or drug selection issues. "
                "Administration errors relate to incorrect route, timing, or missed doses. "
...
with tab3:
    ...
    if ham_lasa_results and ham_lasa_results["total_records"] > 0:
        ...
        if not ham_patient_df.empty:
            st.markdown("##### 👤 HAM/LASA Frequency by Patient")
            st.dataframe(ham_patient_df, use_container_width=True, hide_index=True)
            fig_hl_patient = ham_lasa_patient_chart(ham_patient_df)
            all_charts["ham_lasa_patient"] = ham_lasa_patient_chart(ham_patient_df, for_report=True)
            st.plotly_chart(fig_hl_patient, use_container_width=True)
            render_description(
                "This tally shows how many times each patient received a High Alert or "
                "Look-Alike Sound-Alike medication during the month. Patients appearing "
                "multiple times may warrant closer monitoring or a medication review."
            )
    ...
    if st.button("📥 Generate Word Report", type="primary", use_container_width=True):
        with st.spinner("Generating report... This may take a moment while charts are rendered."):
            try:
                ...
                if not ham_patient_df.empty:
                    report_charts["ham_lasa_patient"] = ham_lasa_patient_chart(
                        ham_patient_df, for_report=True
                    )
                ...
                doc_buffer = generate_report(
                    ...
                    ham_patient_df=ham_patient_df if not ham_patient_df.empty else None,
